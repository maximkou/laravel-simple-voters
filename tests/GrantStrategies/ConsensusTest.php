<?php
/**
 * @author Maksim Khodyrev<maximkou@gmail.com>
 * 12.05.17
 */

namespace Maximkou\SimpleVoters\Tests\GrantStrategies;

use Maximkou\SimpleVoters\GrantStrategies\Consensus;
use Maximkou\SimpleVoters\Tests\BasicTestCase;
use Mockery as m;

/**
 * Class ConsensusTest
 * @package Maximkou\SimpleVoters\Tests\GrantStrategies
 * @see Consensus
 */
class ConsensusTest extends BasicTestCase
{
    use StrategyTrait;

    public function testMoreProVoters()
    {
        $strategy = new Consensus([
            $this->createVoter([
                'edit' => [
                    'supports' => true,
                    'granted' => true
                ],
                'remove' => [
                    'supports' => true,
                    'granted' => true
                ]
            ]),
            $this->createVoter([
                'edit' => [
                    'supports' => true,
                    'granted' => true
                ],
                'remove' => [
                    'supports' => false,
                    'granted' => false
                ]
            ]),
            $this->createVoter([
                'edit' => [
                    'supports' => true,
                    'granted' => false
                ],
                'remove' => [
                    'supports' => false,
                    'granted' => false
                ]
            ]),
        ]);

        self::assertTrue($strategy->isGranted(['edit', 'remove'], null, null));
    }

    public function testMoreContraVoters()
    {
        $strategy = new Consensus([
            $this->createVoter([
                'edit' => [
                    'supports' => true,
                    'granted' => false
                ],
                'remove' => [
                    'supports' => true,
                    'granted' => false
                ]
            ]),
            $this->createVoter([
                'edit' => [
                    'supports' => false,
                    'granted' => false
                ],
                'remove' => [
                    'supports' => true,
                    'granted' => true
                ]
            ]),
        ]);

        self::assertTrue($strategy->isGranted(['edit', 'remove'], null, null));
    }

    public function testSameProAndContraVoters()
    {
        $strategyDefTrue = new Consensus([
            $this->createVoter([
                'edit' => [
                    'supports' => true,
                    'granted' => true
                ],
                'remove' => [
                    'supports' => true,
                    'granted' => true
                ]
            ]),
            $this->createVoter([
                'edit' => [
                    'supports' => true,
                    'granted' => false
                ],
                'remove' => [
                    'supports' => true,
                    'granted' => false
                ]
            ]),
        ], true);
        $strategyDefFalse = new Consensus([
            $this->createVoter([
                'edit' => [
                    'supports' => true,
                    'granted' => true
                ],
                'remove' => [
                    'supports' => true,
                    'granted' => true
                ]
            ]),
            $this->createVoter([
                'edit' => [
                    'supports' => true,
                    'granted' => false
                ],
                'remove' => [
                    'supports' => true,
                    'granted' => false
                ]
            ]),
        ], false);

        self::assertTrue($strategyDefTrue->isGranted(['edit', 'remove'], null, null));
        self::assertFalse($strategyDefFalse->isGranted(['edit', 'remove'], null, null));
    }

    // all voters abstain
    public function testAbstainAllVoters()
    {
        $strategyDefFalse = new Consensus([
            $this->createVoter([
                'edit' => [
                    'supports' => false,
                    'granted' => false
                ],
            ]),
            $this->createVoter([
                'edit' => [
                    'supports' => false,
                    'granted' => false
                ],
            ]),
        ], false);
        $strategyDefTrue = new Consensus([
            $this->createVoter([
                'edit' => [
                    'supports' => false,
                    'granted' => false
                ],
            ]),
            $this->createVoter([
                'edit' => [
                    'supports' => false,
                    'granted' => false
                ],
            ]),
        ], true);

        self::assertFalse($strategyDefFalse->isGranted('edit', null, null));
        self::assertTrue($strategyDefTrue->isGranted('edit', null, null));
    }
}